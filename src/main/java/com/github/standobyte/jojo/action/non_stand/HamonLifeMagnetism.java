...
public class HamonLifeMagnetism extends HamonAction {
	...
	public ActionConditionResult checkSpecificConditions(LivingEntity user, INonStandPower power, ActionTarget target) {
		if (target.getType() == TargetType.BLOCK && user.level.getBlockState(target.getBlockPos()).getBlock() instanceof LeavesBlock || useItemForGlider(user.getMainHandItem()) || useItemForGlider(user.getOffhandItem()) || user instanceof PlayerEntity && !MCUtil.findInInventory(((PlayerEntity) user).inventory, item -> !item.isEmpty() && item.getItem() instanceof BlockItem && ((BlockItem) item.getItem()).getBlock() instanceof LeavesBlock).isEmpty()) {
...
//					world.destroyBlock(blockPos, false);
			...
			if (user instanceof PlayerEntity) {
				...
				if (!useItemForGlider(leavesItem))
					leavesItem = MCUtil.findInInventory(inventory, item -> useItemForGlider(item));
	...
	private boolean useItemForGlider(ItemStack item) {
		return !item.isEmpty() && item.getItem() instanceof BlockItem && ((BlockItem) item.getItem()).getBlock() instanceof LeavesBlock;
	}
