...
public class TimeStopInstant extends StandAction {
	...
	public TimeStopInstant(StandAction.Builder builder, @Nonnull Supplier<TimeStop> baseTimeStopAction, @Nullable Supplier<SoundEvent> blinkSound) {
		this(builder, baseTimeStopAction, blinkSound, false);
	}

	public TimeStopInstant(StandAction.Builder builder, @Nonnull Supplier<TimeStop> baseTimeStopAction, @Nullable Supplier<SoundEvent> blinkSound, boolean teleportBehindEntity) {
		super(builder);
	...
	protected ActionConditionResult checkSpecificConditions(LivingEntity user, IStandPower power, ActionTarget target) {
		...
		if (power.isActive() && power.getStandManifestation() instanceof StandEntity && ((StandEntity) power.getStandManifestation()).getCurrentTask().isPresent()) {
			return ActionConditionResult.NEGATIVE;
		}
...
//		}
//
//		user.level.getEntitiesOfClass(MobEntity.class, user.getBoundingBox().inflate(8), mob -> mob.getTarget() == user && mob.getLookAngle().dot(mob.getEyePosition(1).subtract(blinkPos)) >= 0).forEach(mob -> {
...
//		user.getCapability(LivingUtilCapProvider.CAPABILITY).ifPresent(cap -> cap.hasUsedTimeStopToday = true);
	...
	public Vector3d calcBlinkPos(LivingEntity user, ObjectWrapper<ActionTarget> initialTarget, double maxDistance) {
		...
		if (target.getType() == TargetType.EMPTY) {
			RayTraceResult rayTrace = JojoModUtil.rayTrace(user, maxDistance, entity -> entity instanceof LivingEntity && !(entity instanceof StandEntity && ((StandEntity) entity).getUser() == user));
	...
	void playSound(World world, Entity entity) {
		if (blinkSound != null) {
			...
			if (sound != null) {
				MCUtil.playSound(world, entity instanceof PlayerEntity ? (PlayerEntity) entity : null, entity.getX(), entity.getY(), entity.getZ(), sound, SoundCategory.AMBIENT, 5.0F, 1.0F, TimeStopHandler::canPlayerSeeInStoppedTime);
			}
		}
	}
	...
	protected Vector3d getEntityTargetTeleportPos(Entity user, Entity target) {
		...
		if (teleportBehindEntity) {
			pos = target.position().subtract(Vector3d.directionFromRotation(0, target.yRot).scale(target.getBbWidth() + user.getBbWidth()));
		} else {
